...
1.  What type of plot would you use to display the relationship between `runs` 
    and one of the other numerical variables?
...
```
   ...
   If you knew a team's `at_bats`, would you be comfortable using a linear model to predict the number of runs?
        ...
        A linear model would gives us an estimate of the number of runs. We would however have to check the 
        strength of the relationship between the two varialbles to see how reliable the model would be.
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    There is a weak linear relationship between the at_bats and the runs with a
    big concentration at around 600-700 runs.
    There are a few major outliers at around 850 runs and 5650-5750 at_bats.
...
\[\hat{y}=-85.15663 + 0.34154 * homeruns\]
        ...
        From the slope, ther is a positive relationship between the success of a
        team and its homeruns.
...
```
        Using the model 728 runs would be predicted.
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    about the linearity of the relationship between runs and at-bats?
        ...
        the residuals plot show no apparent pattern and as such we conclude that the
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    The normal probability plot is close to the normal line and thus the near normal
    discribution condition is met.
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    ...
    The constant variability condition seems to be met despite the few values on the
...
-   Choose another traditional variable from `mlb11` that you think might be a 
    ...
    a linear model. At a glance, does there seem to be a linear relationship?
        ...
        The relationship seems to be positive and linear.
...
```
    ...
    The R$^2$ value for at_bats and runs is 0.3505, while that of wins and runs is 0.3381.
    The R$^2$ for the two variables is comparable, although at_bats and runs have a slightly
    higher R$^2$ value.
...
-   Now that you can summarize the linear relationship between two variables, 
    ...
    Remaining variables:
    Hits, home runs, batting average, strikeouts, stolen bases.
...
```
        ...
        Batting average has a the strongest R^2 indicating the most positive linear 
        relationshup with runs.  its has a value of 0.6438.
...
-   Now examine the three newer variables. These are the statistics used by the 
    ...
    These variables are: on-base percentage, slugging percentage, and on-base plus slugging. 
    From the below summary statistics these variables show a stronger relationship compared 
    to the earlier variables.
    On base sluggish seem to be the best predictor overall.
...
```
        ...
        Nearly normal condition: The histogram of residials show a near normal 
...
hist(m10$residuals)
```
        Constant variability condition: The residuals show constant variability across all values.
